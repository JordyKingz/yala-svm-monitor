...
pub struct SlotTracker {
    client: Arc<RpcClientWithFailover>,
    track_leaders: bool,
    finalized_only: bool,
...
impl SlotTracker {
    ...
    pub fn new(
        ...
        Self {
            client: Arc::new(RpcClientWithFailover::new(rpc_url)),
            track_leaders,
            finalized_only,
    ...
    pub async fn get_slot_leaders(&self, slot: u64, limit: u64) -> Result<Vec<String>> {
        match self.client.get_slot_leaders(slot, limit).await {
    ...
    async fn fetch_recent_transactions(&self, slot: u64) -> Result<Vec<TransactionData>> {
        ...
        match self.client.get_signatures_for_address(&solana_sdk::pubkey::Pubkey::default()).await {
            Ok(signatures) => {
    ...
    async fn fetch_recent_accounts(&self, slot: u64) -> Result<Vec<AccountData>> {
        ...
        match self.client.get_account(&system_program).await {
            Ok(account) => {
                Ok(vec![AccountData {
    ...
    async fn update_leaders(&mut self) -> Result<()> {
        if let Some(current_slot) = self.last_slot {
            ...
            match self.client.get_slot_leaders(current_slot, 1).await {
                Ok(leaders) => {
...
pub async fn start_tracking(
    ...
    let mut tracker = SlotTracker::new(rpc_url, track_leaders, finalized_only, update_interval_ms);
